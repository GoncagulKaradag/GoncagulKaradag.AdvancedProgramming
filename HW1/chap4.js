function phi([n00, n01, n10, n11]) {
  ...
  // Analiz fonksiyonunu bizden istenildigi sekilde degistirdim.
  function analyze(min=0) {
   
  
    return [...EVENTS]
    .map(e => ({evt: e, cor: phi(tableFor(e)),num:JOURNAL.map(x=>x.events.filter(a=>a==e).length).filter(varMi=>varMi==1).length}))
